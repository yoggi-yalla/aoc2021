...
def intersection_point(box1, box2):
    _, x_min_1, x_max_1, y_min_1, y_max_1, z_min_1, z_max_1 = box1
    _, x_min_2, x_max_2, y_min_2, y_max_2, z_min_2, z_max_2 = box2
...
for turn_on, x_min, x_max, y_min, y_max, z_min, z_max in instructions:
    ...
    coords = set(
        itertools.product(
            list(range(x_min, x_max+1)),
            list(range(y_min, y_max+1)),
            list(range(z_min, z_max+1))
        )
    )
    if turn_on:
        on_cubes.update(coords)
    else:
        on_cubes.difference_update(coords)
